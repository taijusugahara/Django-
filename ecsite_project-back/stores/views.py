...
class ProductListView(LoginRequiredMixin, ListView):
    ...
    #データ（変数）を渡す #何で検索したか表示するようにするためformのvalue(初期値)に入れてあげる
     ...
     if request.is_ajax:
        ...
        if all([product_id, cart, quantity]):
            CartItems.objects.save_item(
                ...
                cart=cart[0] #get_or_createでタプル方で返ってくるので
            )
...
class ConfirmOrderView(LoginRequiredMixin, TemplateView):
    ...
    @transaction.atomic #db操作を一連の流れまとめてする（あるdb操作でエラーが起きた場合ロールバックして他のdb操作も処理しない）
