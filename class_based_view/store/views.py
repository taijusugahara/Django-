...
class BookListView(ListView):
  ...
  def get_queryset(self):
    qs = super(BookListView,self).get_queryset() #それぞれのインスタンスオブジェクトを内包しているリスト
...
class BookCreateView(CreateView):
  ...
  def form_valid(self,form): #form送信前に実行される
...
class BookUpdateView(SuccessMessageMixin,UpdateView): #success先
  ...
  def get_context_data(self, **kwargs):
    ...
    pictures = Pictures.objects.fillter_by_book(book=self.object)
...
def delete_picture(request,pk):
  ...
  picture.delete()
  #画像をmediaから削除する 本来はインスタンスを削除しても画像は残っているがそれを削除する
  # import os
